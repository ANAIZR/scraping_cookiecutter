...
def get_plague_summary_view(request):
    ...
    if not scientific_name:
        return Response(
            {"error": "Parámetro 'scientific_name' es requerido"}, status=400
        )

    scraper_info = ResumeService.get_species_data_by_name(scientific_name)

    if not scraper_info:
        return Response({"error": "No se encontró información en Species"}, status=404)

    paginator = Pagination()
    paginated_data = paginator.paginate_queryset(scraper_info, request)
...
def get_plague_news(request, cabi_id):
    try:
        ...
        related_news = SpeciesNews.objects.filter(
            scientific_name=species.scientific_name
        )

        paginator = Pagination()
        paginated_news = paginator.paginate_queryset(related_news, request)
        serialized_data = SpeciesNewsSerializer(paginated_news, many=True).data

        return paginator.get_paginated_response(serialized_data)

    except CabiSpecies.DoesNotExist:
        return Response({"error": "CabiSpecies no encontrado"}, status=404)
...
class ScraperURLViewSet(viewsets.ModelViewSet):
    ...
    filterset_class = ScraperURLFilter

class SpeciesViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = (
        Species.objects.select_related("scraper_source")
        .distinct()
        .order_by("-created_at")
    )
...
class SpeciesCABIViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = (
        CabiSpecies.objects.select_related("scraper_source")
        .distinct()
        .order_by("-created_at")
    )
    ...
    filter_backends = [DjangoFilterBackend]
    filterset_class = CabiSpeciesFilter
...
class ReportComparisonDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        return ReportComparison.objects.filter(scraper_source=scraper_source).order_by(
            "-comparison_date"
        )
    ...
    def get_latest_scraping_report(self, scraper_source):
        ...
        latest_report = collection.find_one(
            {"url": scraper_source.url}, sort=[("scraping_date", -1)]
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if comparison:
            if not any(
                [
                    comparison.info_agregada,
                    comparison.info_eliminada,
                    comparison.info_modificada,
                ]
            ):
                return Response(
        ...
        logger.info(
            f"❌ No se encontró comparación ni reporte para ScraperURL ID: {scraper_id}"
        )
        return Response(
            {"message": "No se encontró comparación ni reporte para esta URL"},
...
class SpeciesSubscriptionViewSet(viewsets.ModelViewSet):
        ...
        if user_id:
            if not self.request.user.is_staff:
                raise PermissionDenied(
                    "No tienes permisos para ver otras suscripciones."
                )
            return SpeciesSubscription.objects.filter(user_id=user_id).order_by(
                "-created_at"
            )

        return SpeciesSubscription.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
    ...
    def destroy(self, request, *args, **kwargs):
        instance = get_object_or_404(
            SpeciesSubscription, id=kwargs["pk"], user=request.user
        )
