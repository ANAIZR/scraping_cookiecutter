...
def scraper_npdn(url, sobrenombre):
    logger = get_logger("NPDN")
    ...
    def scrape_page(url, depth):
        ...
        try:
            ...
            if depth >= 2:
                ...
                # main_content = soup.find("div", class_=["main-content", "home-widgets"])
                main_content = soup.find("div", class_=["main-content"])
                if main_content:
                    nonlocal all_scraper
                    page_text = main_content.get_text(separator=" ", strip=True)
                    print("scraper by quma", page_text)
            ...
            for link in soup.find_all("a", href=True):
                ...
                if (
                    urlparse(full_url).netloc == "www.npdn.org"
