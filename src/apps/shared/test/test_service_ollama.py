...
class TestOllamaService:
    ...
    def test_process_document_already_processed(self, mocker, mock_mongo_collection):
        mock_mongo_collection.find_one.return_value = {"processed": True}
        service = OllamaService()
        service.text_to_json = mocker.Mock()

        doc = {"_id": "123", "contenido": "sample content", "source_url": "https://example.com"}
        service.process_document(doc)

        mock_mongo_collection.find_one.assert_called_once_with({"_id": "123", "processed": True})
