...
class TestScraperAPIView:
    ...
    @pytest.mark.django_db
    def test_scraper_url_serializer_estado_fallido(self):
        scraper = ScraperURL.objects.create(
            url="https://example.com",
            sobrenombre="Test Scraper",
            time_choices=1,
    ...
    @pytest.mark.django_db
    def test_report_comparison_serializer(self):
        scraper_source = ScraperURL.objects.create(url="https://example.com", sobrenombre="Test Scraper")
    ...
    @pytest.mark.django_db
    def test_scraper_url_list(api_client, admin_user):
        api_client.force_authenticate(user=admin_user)
        ScraperURL.objects.create(url="https://example.com", sobrenombre="Test Scraper")

        response = api_client.get("/api/scraper_urls/")
        assert response.status_code == 200
        assert len(response.data["results"]) > 0

    @pytest.mark.django_db
    def test_species_list(api_client, admin_user):
        api_client.force_authenticate(user=admin_user)
        response = api_client.get("/api/species/")
        assert response.status_code == 200

    @pytest.mark.django_db
    def test_report_comparison_not_found(api_client, admin_user):
        api_client.force_authenticate(user=admin_user)

        response = api_client.get("/api/report_comparison/9999/")
        assert response.status_code == 404

    @pytest.mark.django_db
    def test_species_subscription_create(api_client, admin_user):
        api_client.force_authenticate(user=admin_user)
        data = {
            "name_subscription": "Test Subscription",
        ...
        response = api_client.post("/api/species_subscriptions/", data)
